...
def load_merge_requests(json_path="data/transformers/merge_requests_transformed.json"):
    ...
    try:
        ...
        for mr in merge_requests:
            try:
                # Formatage des dates
                mr["created_at"] = datetime.fromisoformat(mr["created_at"]) if mr.get("created_at") else None
                mr["updated_at"] = datetime.fromisoformat(mr["updated_at"]) if mr.get("updated_at") else None
                mr["merged_at"] = datetime.fromisoformat(mr["merged_at"]) if mr.get("merged_at") else None

                if not all(k in mr for k in ("id", "title", "created_at")):
                    print(f"[⚠️] Merge request ignorée : clé(s) manquante(s) dans {mr.get('id')}")
                    continue
